...
export default function ArticleDetailPage() {
  ...
  const fetchRelatedArticles = useCallback(
    async (category: string, currentId: string) => {
      try {
        const res = await fetch(`${API_BASE}/stories/get-stories`);
        if (!res.ok) throw new Error("Failed to fetch related story");
        const json = await res.json();
        const allstory = json.Stories || json.data || [];

        const filtered = allstory
          .filter(
            (n: any) =>
  ...
  const fetchArticleDetail = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`${API_BASE}/stories/getMagazineId/${params.id}`);
      if (!res.ok) throw new Error("Article not found");
      const json = await res.json();
      const a = json.Stories || json.data;
  ...
  if (!article)
    return (
      <div className="bg-background min-h-screen text-center py-20">
        ...
        <Link href="/stories" className="text-accent underline font-bold">
          Back to story
        </Link>
      </div>
    );

  // Generate dynamic SEO for this story
  const storySeo = seoConfigurations.createStorySeo(
    article.title,
    article.description,
    article.author,
    article.date,
      ...
      <div className="bg-background min-h-screen selection:bg-accent/30">
        ...
        <section className="bg-card/30 border-b border-border">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            ...
            >
              <ChevronLeft className="w-3 h-3" /> Back to storyroom
            </Link>
          </div>
        </section>
